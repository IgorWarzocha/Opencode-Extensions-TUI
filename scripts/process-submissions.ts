...
async function fetchReadme(repositoryUrl: string): Promise<string> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (!repo) {
        throw new Error(`Could not extract repository name from: ${repositoryUrl}`);
      }
      ...
      throw new Error(`README not found in main or master branch for ${owner}/${cleanRepo}`);
      
    } catch (error) {
      console.error(`Attempt ${attempt} failed for ${repositoryUrl}:`, error instanceof Error ? error.message : error);
...
async function processSubmission(submissionPath: string): Promise<void> {
  ...
  if (!extension.readme || extension.readme.trim() === "") {
    ...
    try {
      ...
      console.log(`✅ README fetched successfully (${readmeContent.length} chars)`);
      
    } catch (error) {
      console.error(`❌ Failed to fetch README:`, error instanceof Error ? error.message : error);
    ...
    extension.category = CATEGORY_MAP[category];
    console.log(`📝 Already has README (${extension.readme.length} chars), updating category`);
  }
...
async function main() {
  ...
  try {
    ...
    console.log(`📋 Found ${submissionFiles.length} submission(s) to process:\n`);
    ...
    for (let i = 0; i < submissionFiles.length; i++) {
      ...
      try {
        ...
        if (i < submissionFiles.length - 1) {
          console.log(`⏱️  Waiting ${rateLimitDelay / 1000}s before next submission...`);
          await sleep(rateLimitDelay);
        }
        
      } catch (error) {
        console.error(`💥 Failed to process ${submissionPath}:`, error instanceof Error ? error.message : error);
